...
float4 FinalScenePassPS(uniform const bool use_dof, uniform const int use_hdr, uniform const bool use_auto_exp, float2 texCoord: TEXCOORD0) : COLOR {
	...
	if(use_hdr > 0) {
		...
		// if(use_auto_exp)
		// {
			float greyscale = dot(color.rgb, float3(0.59, 0.59, 0.11));
	...
	if(paletteChoice == 1){ // default
		float3 hsv = RGBtoHSV(color.rgb);
		float3 saturated = hsv;
		// saturated.y *= 1.25f;
		saturated.y *= 1.20f;
		color.rgb = lerp(HSVtoRGB(saturated), HSVtoRGB(hsv), hsv.y);
		float greyscale = dot(color.rgb, float3(0.59, 0.59, 0.59));
		color.rgb = lerp(greyscale, color.rgb,  0.90);
	}
	else if(paletteChoice == 2){ //cloudy
		//greyscale
		float greyscale = dot(color.rgb, float3(0.59, 0.59, 0.59));
		color.rgb = lerp(greyscale, color.rgb,  0.85);
	}
	else if(paletteChoice == 3){ //more cloudy
		//greyscale
		float greyscale = dot(color.rgb, float3(0.59, 0.59, 0.59));
		color.rgb = lerp(greyscale, color.rgb,  0.70);
	}
	else if(paletteChoice == 4){ //night
		//greyscale
		float greyscale = dot(color.rgb, float3(0.59, 0.59, 0.59));
		color.rgb = lerp(greyscale, color.rgb,  0.70);
		...
		float3 value = hsv;
		value.z *= 0.915f;
		color.rgb = lerp(HSVtoRGB(value), HSVtoRGB(hsv), hsv.z);
	}
	else if(paletteChoice == 5){ //interior
		//greyscale
		float greyscale = dot(color.rgb, float3(0.59, 0.59, 0.59));
		color.rgb = lerp(greyscale, color.rgb,  0.875);

		// color.rgb = ((color.rgb - 0.5) * 1.5f) + 0.5;//increase contrast
		float3 hsv = RGBtoHSV(color.rgb);
		float3 value = hsv;
		value.z *= 0.925f;
		color.rgb = lerp(HSVtoRGB(value), HSVtoRGB(hsv), hsv.z);
