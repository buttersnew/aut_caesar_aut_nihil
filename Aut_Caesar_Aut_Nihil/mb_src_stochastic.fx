...
//PARALLAX SECTION
	...
	float height = stochasticTex2D(EnvTextureSampler, parallaxcoords).a;
	float offset = height * volume + bias;
...
//PARALLAX END
	...
	//float4 tex_col = tex2D(MeshTextureSampler, In.Tex0.xy);
	float4 tex_col = stochasticTex2D(MeshTextureSampler, In.Tex0.xy);
	//INPUT_TEX_GAMMA(tex_col.rgb);
	...
	tex_col.rgb = lerp(tex_col.rgb*float3(0.8,0.75,0.65), tex_col.rgb*1.30,1-stochasticTex2D(EnvTextureSampler, parallaxcoords, false).a);
	//
	...
	float3 normal = (2.0f * stochasticTex2D(NormalTextureSampler, In.Tex0.xy * map_normal_detail_factor, false).rgb - 1.0f);
	float3 normalpara = (2.0f * stochasticTex2D(EnvTextureSampler, parallaxcoords, false).rgb - 1.0f);
...
technique new_map_shader
{
	pass P0
	{
		...
		PixelShader = compile ps_3_0 ps_new_map(PCF_NONE);
	}
}
...
//height *= 0.5;
	...
	//parallax darkening
	tex_col.rgb = lerp(tex_col.rgb*float3(0.8,0.75,0.65), tex_col.rgb*1.30,1-tex2D(EnvTextureSampler, parallaxcoords).a);
	//tex_col.rgb = lerp(tex_col.rgb*float3(0.8,0.75,0.65), tex_col.rgb*1.30,1-stochasticTex2D(EnvTextureSampler, parallaxcoords).a);
	//
	...
	float3 normal = (2.0f * tex2D(NormalTextureSampler, In.Tex0.xy * map_normal_detail_factor).rgb - 1.0f);
	float3 normalpara = (2.0f * tex2D(EnvTextureSampler, parallaxcoords).rgb - 1.0f);
	//float3 normal = (2.0f * stochasticTex2D(NormalTextureSampler, In.Tex0.xy * map_normal_detail_factor).rgb - 1.0f);
	//float3 normalpara = (2.0f * stochasticTex2D(EnvTextureSampler, parallaxcoords).rgb - 1.0f);
